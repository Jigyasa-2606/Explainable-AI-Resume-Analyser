...
from backend.resume_analyzer import TRAINED_MODEL_PATH, extract_scoring_features
...
def build_feature_matrix(rows: list[dict[str, str]], use_transformer: bool = False) -> tuple[list[list[float]], list[float], list[str], str]:
    feature_dicts = [
        extract_scoring_features(row["resume"], row["job_description"], use_transformer=use_transformer)
        for row in rows
    ]
...
def train_model(
    ...
    features, targets, feature_names, target_scale = build_feature_matrix(rows, use_transformer=use_transformer)
    ...
    artifact = {
        ...
        "use_transformer_features": use_transformer,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--use-transformer",
        action="store_true",
        help="Include SBERT semantic features (requires sentence-transformers).",
    )
    args = parser.parse_args()

    use_transformer = args.use_transformer or os.getenv("USE_TRANSFORMER_EMBEDDINGS", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
