...
from .resume_analyzer import (
    ...
    infer_resume_job_queries,
    rank_jobs_for_resume,
    sanitize_job_query_text,
    skill_graph_explanations,
    trained_model_info,
)
...
COUNTRY_LOCATION_TERMS = {
    ...
    "us": {"united states", "usa", "u s", "remote", "anywhere", "worldwide"},
    "gb": {"united kingdom", "uk", "england", "london", "remote", "anywhere", "worldwide"},
    "ca": {"canada", "toronto", "vancouver", "remote", "anywhere", "worldwide"},
    "au": {"australia", "sydney", "melbourne", "remote", "anywhere", "worldwide"},
    "sg": {"singapore", "remote", "anywhere", "worldwide"},
}
...
def health() -> dict[str, Any]:
    return {"status": "ok", "model": trained_model_info()}
...
async def live_jobs(
    ...
    if resume_file is not None:
        ...
        extracted_from_file = extract_text_from_upload(resume_file.filename or "", content).strip()
        if extracted_from_file:
            resolved_resume_text = extracted_from_file

    if not resolved_resume_text.strip():
        if upload_bytes > 0 and not extracted_from_file:
            raise HTTPException(
                status_code=400,
                detail=(
                    "Could not read text from your resume file. "
                    "Try a .txt or .docx export, or paste your resume text in the box. "
                    "Image-only PDFs need OCR or a selectable-text version."
                ),
            )
    ...
    suggested_queries = [sanitize_job_query_text(q) for q in suggested_queries]
    suggested_queries = [q for q in suggested_queries if len(q) >= 3]

    role_queries = _parse_selected_roles(selected_roles)
    combined_roles = sanitize_job_query_text(_combine_role_queries(role_queries))
    raw_query = query.strip() or combined_roles or ""

    search_query = sanitize_job_query_text(raw_query)
    if not search_query:
    ...
    search_query = sanitize_job_query_text(search_query)
    if not search_query:
        search_query = "software developer"
    ...
    live_job_results = _filter_country(live_job_results, country)
...
def _serialize_ranked_job(item: dict[str, Any], live_job: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "analysis": {
            ...
            "first_pass_score": analysis.get("first_pass_score"),
            "stage2_score": analysis.get("stage2_score"),
...
def _fetch_jobs_for_provider(
    ...
    try:
        ...
        if normalized_provider == "internshala":
            return fetch_apify_internshala_jobs(query, limit=limit, country=country), []
...
def _fetch_all_providers(
    ...
    provider_calls = [
        ...
        ("Internshala", lambda: fetch_apify_internshala_jobs(query, limit=limit, country=country)),
...
def _filter_country(live_jobs: list[LiveJob], country: str) -> list[LiveJob]:
    country_code = country.lower()
    include_terms = COUNTRY_LOCATION_TERMS.get(country_code)
    if not include_terms:
        return live_jobs

    exclude_terms = COUNTRY_EXCLUSION_TERMS.get(country_code, set())
    filtered_jobs = []
    for job in live_jobs:
        location_text = " ".join([job.location, job.description, job.title]).lower()
        if any(term in location_text for term in exclude_terms):
            continue
        if not job.location.strip() or any(term in location_text for term in include_terms):
            filtered_jobs.append(job)

    return filtered_jobs or live_jobs
