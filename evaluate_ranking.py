...
def _pair_lists(group: list[dict[str, object]]) -> tuple[list[str], list[str]]:
    resumes = [str(row["resume"]) for row in group]
    jobs = [str(row["job_description"]) for row in group]
    return resumes, jobs


def score_first_pass_group(group: list[dict[str, object]]) -> list[float]:
    resumes, jobs = _pair_lists(group)
    scores, _ = first_pass_scores_for_pairs(resumes, jobs)
    return scores
...
def score_ltr_group(group: list[dict[str, object]]) -> list[float]:
    ...
    return score_first_pass_group(group)


def score_two_stage_group(group: list[dict[str, object]], top_k: int) -> tuple[list[float], str]:
    resumes, jobs = _pair_lists(group)
    scores, method, _ = two_stage_scores_for_pairs(resumes, jobs, top_k=top_k)
...
def evaluate(
    ...
    two_stage_methods: list[str] = []
    two_stage_scores: list[list[float]] = []
    for group in groups:
        scores, method = score_two_stage_group(group, two_stage_top_k)
        two_stage_scores.append(scores)
        two_stage_methods.append(method)

    method_scores: dict[str, list[list[float]]] = {
        "rule_formula": [[score_rule_based(row) for row in group] for group in groups],
        "rf_first_pass": [score_first_pass_group(group) for group in groups],
        "ltr_ranker": [score_ltr_group(group) for group in groups],
        "two_stage": two_stage_scores,
    }
    if include_sbert:
    ...
    two_stage_applied = any(method.startswith("two_stage_") for method in two_stage_methods)
    ...
    return {
        ...
        "k_values": k_values,
        "two_stage_top_k": two_stage_top_k,
        "two_stage_method": two_stage_methods[0] if two_stage_methods else "unavailable",
        "two_stage_applied": two_stage_applied,
        "transformer_requested": include_sbert,
        "transformer_available": transformer_available(),
        "rf_model_available": bool(model_info.get("available")),
        "rf_model_error": model_info.get("load_error"),
        "metrics": method_metrics,
        "improvements_vs_rule_formula": improvements,
    }


def _metric_order(k_values: list[int]) -> list[str]:
    ordered: list[str] = []
    for k in k_values:
        ordered.extend([f"ndcg@{k}", f"precision@{k}", f"recall@{k}"])
    return ordered
...
def print_report(result: dict[str, object]) -> None:
    ...
    print(f"Two-stage method: {result['two_stage_method']} (top_k={result['two_stage_top_k']})")
    print(f"SBERT available: {result['transformer_available']}")
    print(f"Random Forest model available: {result['rf_model_available']}")
    if result.get("rf_model_error"):
        print(f"Note: RF model did not load ({result['rf_model_error']}). Stage 1 uses the TF-IDF + skills heuristic.")
    if not result["transformer_available"]:
        print("Note: install sentence-transformers to enable the SBERT second pass. Stage 1 (RF) still runs.")
    elif not result["two_stage_applied"]:
        print("Note: sentence-transformers is installed but stage-2 scores were unavailable.")
    print()

    print("Primary ranking metrics")
    metrics: dict[str, dict[str, float]] = result["metrics"]  # type: ignore[assignment]
    header = f"{'method':<18}" + "".join(f"{name:>12}" for name in PRIMARY_METRICS)
    print(header)
    for method, values in metrics.items():
        cells = "".join(f"{values.get(name, 0.0):12.4f}" for name in PRIMARY_METRICS)
        print(f"{method:<18}{cells}")
    print()

    k_values: list[int] = result["k_values"]  # type: ignore[assignment]
    order = _metric_order(k_values)
    for method, values in metrics.items():
        print(method)
        for metric in order:
            if metric in values:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate ranking quality with NDCG@5 / NDCG@10 (plus Precision@K and Recall@K)."
    )
    ...
    parser.add_argument("--two-stage-top-k", type=int, default=30, help="Stage-2 rerank window (SBERT / cross-encoder).")
    parser.add_argument("--include-sbert", action="store_true", help="Also evaluate a pure SBERT heuristic formula.")
    ...
    result = evaluate(
        ...
        two_stage_top_k=args.two_stage_top_k,
    )
    print_report(result)
