...
function App() {
  ...
  async function findJobs(event) {
    ...
    if (!pasted && !resumeFile) {
      setError("Add your resume: upload a PDF, DOCX, or TXT file, or paste your text in the box below.");
      setLoading(false);
      return;
    }

    if (resumeFile && resumeFile.size === 0) {
      setError("The selected file is empty. Pick another file or paste your resume text.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(formatApiError(payload) || "Could not fetch matching jobs.");
      setResult(payload);
  ...
  async function wakeBackend(base) {
    for (let attempt = 1; attempt <= 4; attempt += 1) {
      try {
        ...
        // Render free tier can take up to a minute to wake from sleep.
      }
  ...
  async function apiFetch(path, options, retries = 3) {
    ...
    const target = bases.length ? bases.join(" or ") : "the local Vite API proxy";
    const hint = API_BASE
      ? "The Render backend may be waking up (free tier sleeps after 15 min). Open /health in a new tab, wait 60 seconds, then try again."
      : "Set VITE_API_BASE on Vercel to your Render URL and redeploy.";
    throw new Error(`Could not connect to backend at ${target}. ${hint} ${lastError?.message || ""}`.trim());
  }
        ...
        <aside className="panel">
          ...
          <label>
            ...
            <select value={provider} onChange={(e) => setProvider(e.target.value)}>
              ...
              <option value="internshala">Internshala via Apify</option>
              <option value="remotive">Remotive (no key needed)</option>
            </select>
          </label>

          {(provider === "jsearch" || provider === "adzuna" || provider === "serpapi" || provider === "jooble" || provider === "internshala" || provider === "all") && (
            <>
              <p className="hint">
                {provider === "all"
                  ? "🔒 Searches JSearch, Adzuna, SerpAPI, Jooble, Internshala, and Remotive together. API keys stay in the backend."
                  : provider === "jsearch"
                    ...
                    : provider === "adzuna"
                      ...
                      : provider === "serpapi"
                        ...
                        : provider === "jooble"
                          ...
                          : "🔒 Apify token and Internshala actor ID are configured in the backend."}
              </p>
